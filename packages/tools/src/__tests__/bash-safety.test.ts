...
describe("runSafetyCheck — level: off", () => {
  it("allows any command including sudo", () => {
    expect(() => runSafetyCheck("sudo rm -rf /", off)).not.toThrow();
  });

  it("allows fork bomb pattern", () => {
    expect(() => runSafetyCheck(":(){:|:&};:", off)).not.toThrow();
  });

  it("allows dd writing to device", () => {
    expect(() => runSafetyCheck("dd if=/dev/zero of=/dev/sda", off)).not.toThrow();
  });
});

describe("runSafetyCheck — sudo block (basic/standard/strict)", () => {
  it("blocks sudo in basic mode", () => {
    expect(() => runSafetyCheck("sudo apt-get install vim", basic)).toThrow(
      expect.objectContaining({ code: "SUDO_NOT_ALLOWED" }),
    );
  });

  it("blocks sudo in standard mode", () => {
    expect(() => runSafetyCheck("sudo ls /", standard)).toThrow(
      expect.objectContaining({ code: "SUDO_NOT_ALLOWED" }),
    );
  });

  it("blocks sudo in strict mode", () => {
    expect(() => runSafetyCheck("sudo cat /etc/passwd", strict)).toThrow(
      expect.objectContaining({ code: "SUDO_NOT_ALLOWED" }),
    );
  });

  it("blocks pseudosudo-like commands containing the word sudo", () => {
    expect(() => runSafetyCheck("echo sudo", basic)).toThrow(
      expect.objectContaining({ code: "SUDO_NOT_ALLOWED" }),
    );
  });
});
...
describe("runSafetyCheck — dangerous command blocklist", () => {
  it("blocks rm -rf /", () => {
    expect(() => runSafetyCheck("rm -rf /", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks rm -fr /", () => {
    expect(() => runSafetyCheck("rm -fr /", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });
  ...
  it("blocks rm --no-preserve-root", () => {
    expect(() => runSafetyCheck("rm --no-preserve-root -rf /", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks dd writing to a block device", () => {
    expect(() => runSafetyCheck("dd if=/dev/zero of=/dev/sda bs=512", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks mkfs", () => {
    expect(() => runSafetyCheck("mkfs.ext4 /dev/sdb1", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks wipefs", () => {
    expect(() => runSafetyCheck("wipefs -a /dev/sdb", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks sgdisk -Z partition wipe", () => {
    expect(() => runSafetyCheck("sgdisk -Z /dev/sda", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks shred on a device", () => {
    expect(() => runSafetyCheck("shred -v /dev/sda", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks insmod", () => {
    expect(() => runSafetyCheck("insmod evil.ko", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks rmmod", () => {
    expect(() => runSafetyCheck("rmmod ext4", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks overwriting /etc/passwd", () => {
    expect(() => runSafetyCheck("echo root:x:0:0 > /etc/passwd", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("blocks overwriting /etc/shadow", () => {
    expect(() => runSafetyCheck("echo '' > /etc/shadow", basic)).toThrow(
      expect.objectContaining({ code: "DANGEROUS_COMMAND" }),
    );
  });

  it("allows safe rm commands", () => {
    expect(() => runSafetyCheck("rm -rf /home/user/project/dist", basic)).not.toThrow();
  });

  it("allows dd reading from a device (no of=/dev/)", () => {
    expect(() => runSafetyCheck("dd if=/dev/urandom of=/tmp/random bs=1k count=10", basic)).not.toThrow();
  });

  it("allows ls /", () => {
    expect(() => runSafetyCheck("ls /", basic)).not.toThrow();
  });

  it("allows cat /etc/hostname", () => {
    expect(() => runSafetyCheck("cat /etc/hostname", basic)).not.toThrow();
  });
});
...
describe("runSafetyCheck — path restriction (standard)", () => {
  ...
  it("allows commands without paths when no workspaceRoot provided", () => {
    expect(() => runSafetyCheck("cat /etc/secret.conf", standard)).not.toThrow();
  });
...
describe("runSafetyCheckAsync — strict mode with tree-sitter AST", () => {
  ...
  it("allows a safe echo command", async () => {
    await expect(runSafetyCheckAsync("echo hello world", strict, WORKSPACE)).resolves.toBeUndefined();
  });
  ...
  it("blocks mkfs via AST node type check", async () => {
    await expect(runSafetyCheckAsync("mkfs.ext4 /dev/sdb1", strict, WORKSPACE)).rejects.toMatchObject({
  ...
  it("blocks path outside workspace in strict mode", async () => {
    await expect(
      runSafetyCheckAsync("cat /etc/passwd", strict, WORKSPACE),
